"""Training script for ResNet-MLP Baseline"""
import os
import sys
import torch
...
def setup_logger(log_dir):
    ...
    log_file = log_dir / f"train_resnet_mlp_{timestamp}.log"
    logger = logging.getLogger('train_resnet_mlp')
...
def main():
    ...
    parser.add_argument('--config', type=str, default='baselines/resnet_mlp/config_resnet_mlp.yaml')
    ...
    logger = setup_logger(config['logging'].get('log_dir', 'baselines/resnet_mlp/logs'))

    model = ResNetMLP(config).cuda()
    total = sum(p.numel() for p in model.parameters())
    logger.info(f"ResNet-MLP 参数: {total:,}")
