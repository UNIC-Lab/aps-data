"""Training script for ViT Regression Baseline"""
import os
import sys
import torch
...
def setup_logger(log_dir):
    ...
    log_file = log_dir / f"train_vit_reg_{timestamp}.log"
    logger = logging.getLogger('train_vit_reg')
...
def train_epoch(model, train_loader, optimizer, scheduler, config, epoch, use_amp):
    model.train()
    total_loss = 0
...
@torch.no_grad()
def validate(model, val_loader, config, use_amp):
    model.eval()
    total_loss = 0
    dtype = torch.bfloat16 if config['training']['mixed_precision'] == 'bf16' else torch.float16
...
def main():
    ...
    parser.add_argument('--config', type=str, default='baselines/vit_reg/config_vit_reg.yaml')
    ...
    logger = setup_logger(config['logging'].get('log_dir', 'baselines/vit_reg/logs'))

    model = ViTRegression(config).cuda()

    total = sum(p.numel() for p in model.parameters())
    logger.info(f"ViT Regression 参数: {total:,}")
    ...
    for epoch in range(1, config['training']['epochs'] + 1):
        train_loss = train_epoch(model, train_loader, optimizer, scheduler, config, epoch, use_amp)
        val_loss = validate(model, val_loader, config, use_amp)
